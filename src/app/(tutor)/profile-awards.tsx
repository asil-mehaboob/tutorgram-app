...
import { ArrowLeft, Plus, PencilSimple, Trash, Trophy, CalendarBlank } from 'phosphor-react-native';
...
type FormFields = Omit<TutorAward, 'id' | 'description'>;

const EMPTY: FormFields = { title: '', issuer: null, date: null };
...
export default function ProfileAwards() {
  ...
  const createM = useMutation({ mutationFn: () => createAward(fullForm()), onSuccess: () => { qc.invalidateQueries({ queryKey: ['tutor-awards'] }); closeForm(); } });
  const updateM = useMutation({ mutationFn: () => updateAward(editing!.id, fullForm()), onSuccess: () => { qc.invalidateQueries({ queryKey: ['tutor-awards'] }); closeForm(); } });
  ...
  function openEdit(item: TutorAward) {
    setEditing(item);
    setForm({ title: item.title, issuer: item.issuer, date: item.date });
    setDescription(item.description ?? '');
    setShowForm(true);
  }
  function closeForm() { setShowForm(false); setEditing(null); setForm(EMPTY); setDescription(''); }

  function confirmDelete(item: TutorAward) {
    showDialog({
      title: 'Delete award',
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            <Trophy size={40} color={theme.border} weight="thin" />
            <Text style={[styles.empty, { color: theme.textSecondary }]}>No awards added yet</Text>
          </View>
        }
          ...
          <View style={[styles.modal, { backgroundColor: theme.background }]}>
            ...
            >
              <Input
                ...
                placeholder="e.g. Best Instructor of the Year"
              />
              <Input
                label="Issuer / Publication"
                value={form.issuer ?? ''}
                onChangeText={(v) => setForm({ ...form, issuer: v || null })}
                placeholder="Organization that gave the award"
              />
              <DateField
                label="Date"
                value={form.date ?? ''}
                onChange={(v) => setForm({ ...form, date: v || null })}
              />
              <RichField
                label="Description"
                value={description}
              ...
              />
              <Button
                label={editing ? 'Save Changes' : 'Add Award'}
