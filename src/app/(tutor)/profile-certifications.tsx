...
import { ArrowLeft, Plus, PencilSimple, Trash, Certificate, CalendarBlank } from 'phosphor-react-native';
...
const EMPTY: Omit<TutorCertification, 'id'> = {
  name: '', issuer: '', issueDate: null, expiryDate: null, credentialId: null, credentialUrl: null,
};
...
export default function ProfileCertifications() {
  ...
  function openEdit(item: TutorCertification) {
    setEditing(item);
    setForm({ name: item.name, issuer: item.issuer, issueDate: item.issueDate, expiryDate: item.expiryDate, credentialId: item.credentialId, credentialUrl: item.credentialUrl });
    setShowForm(true);
  }
  function closeForm() { setShowForm(false); setEditing(null); setForm(EMPTY); }

  function confirmDelete(item: TutorCertification) {
    showDialog({
  ...
  const dateRange = (item: TutorCertification) => {
    const issue = formatDate(item.issueDate);
    const expiry = formatDate(item.expiryDate);
    if (!issue && !expiry) return '';
    if (!expiry) return `Issued ${issue}`;
    if (!issue) return `Expires ${expiry}`;
    return `${issue} – ${expiry}`;
  };
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyWrap}>
            <Certificate size={40} color={theme.border} weight="thin" />
            <Text style={[styles.empty, { color: theme.textSecondary }]}>No certifications added yet</Text>
          </View>
        }
        renderItem={({ item, index }) => (
          <View style={[
            ...
            <Text style={[styles.cardIssuer, { color: theme.textSecondary }]}>{item.issuer}</Text>
            ...
            {!!item.credentialId && (
              <Text style={[styles.cardMeta, { color: theme.textSecondary }]}>ID: {item.credentialId}</Text>
            )}
          </View>
        )}
      />
          ...
          <View style={[styles.modal, { backgroundColor: theme.background }]}>
            ...
            >
              ...
              <DateField
                ...
                onChange={(v) => setForm({ ...form, issueDate: v || null })}
              />
              <DateField
                label="Expiry Date"
              ...
              <Input
                ...
                onChangeText={(v) => setForm({ ...form, credentialId: v || null })}
                placeholder="Credential ID (optional)"
              />
              <Input
                label="Credential URL"
                value={form.credentialUrl ?? ''}
                onChangeText={(v) => setForm({ ...form, credentialUrl: v || null })}
                placeholder="https://..."
                autoCapitalize="none"
              />
              <Button
                label={editing ? 'Save Changes' : 'Add Certification'}
...
const styles = StyleSheet.create({
  ...
  actionBtn: { padding: 2 },
  cardIssuer: { fontSize: 13, fontFamily: Fonts.regular, marginBottom: 3 },
